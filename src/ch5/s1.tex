...
\wl{5.1.9} If $V$ is an IP space and $(\vec{v}_i)_{i \in I}$ is an orthonormal basis. Then we can write all $\vec{w} \in V$ as $\vec{w} = \sum_{i \in I} \lambda_i \vec{v}_i$, where $\lambda_i = (\vec{w}, \vec{v}_i)$.
